...
def main(args):
    ...
    if args.use_smp_implementation < 1 < tsm.state.tensor_parallel_degree:
        ...
        if global_rank == 0:
            logger.info(
                "Tensor parallelism (TP) is enabled as tensor_parallel_degree is set to {tsm.state.tensor_parallel_degree} (>1). Switching use_smp_implementation to 1 so we can use SMP optimized implementation for TP.",
